...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 xl:p-12 mb-10 w-full">
    <div class="flex items-center gap-4 mb-8 pb-6 border-b border-gray-100">
        <a href="{{ route('event.index') }}" class="p-2.5 bg-[#fcf5f5] text-[#f07b32] hover:bg-[#f07b32] hover:text-white rounded-xl transition-all">
        ...
        <h2 class="text-2xl font-extrabold text-[#3d3d3d]">Edit Event & Proposal</h2>
    </div>
    ...
    <form action="{{ route('event.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-3xl">
        ...
        <div>
            <label class="block text-[11px] font-extrabold text-[#3d3d3d] uppercase tracking-widest mb-2.5">Kategori / Jenis Event</label>
        ...
        <div>
            ...
            <textarea name="description" rows="4" 
                      class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#f07b32]/20 focus:border-[#f07b32] bg-gray-50/30 text-[13px] font-medium text-[#3d3d3d] focus:outline-none transition-all resize-none" required>{{ old('description', $event->description) }}</textarea>
        </div>

        <div>
            <label class="block text-[11px] font-extrabold text-[#3d3d3d] uppercase tracking-widest mb-2.5">Ganti Poster Event (Opsional, Format .JPG/.PNG)</label>
            
            @if($event->poster_image)
                <div class="mb-4 p-2 border border-gray-100 rounded-2xl bg-white inline-block">
                    <img src="{{ asset('storage/' . $event->poster_image) }}" alt="Poster Lama" class="h-32 rounded-xl object-cover shadow-sm">
                </div>
            @endif
            ...
            <p class="text-[11px] text-gray-400 font-bold mt-2">Biarkan kosong jika tidak ingin mengganti poster.</p>
        </div>

        <div>
            <label class="block text-[11px] font-extrabold text-[#3d3d3d] uppercase tracking-widest mb-2.5">Ganti Proposal (Opsional, Format .PDF)</label>
